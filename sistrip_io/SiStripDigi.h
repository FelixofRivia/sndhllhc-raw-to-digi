...
inline SiStripDigi::SiStripDigi(uint16_t strip, uint16_t adc, uint32_t fed_key, uint32_t time, const DetectorInfo& detinfo) : fed_key_(fed_key), time_(time), adc_(adc), is_valid_(true) {
  detector_id_ =
