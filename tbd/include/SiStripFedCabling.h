...
public:
  ...
  void print(std::stringstream& ss, const TrackerTopology* trackerTopo) const { printDebug(ss, trackerTopo); }
  ...
  void summary(std::stringstream& ss, const TrackerTopology* trackerTopo) const { printSummary(ss, trackerTopo); }
...
inline SiStripFedCabling::ConnsConstIter SiStripFedCabling::ConnsRange::begin() const { return range_.begin(); }

inline SiStripFedCabling::ConnsConstIter SiStripFedCabling::ConnsRange::end() const { return range_.end(); }

inline SiStripFedCabling::ConnsConstIterRange SiStripFedCabling::ConnsRange::range() const { return range_; }
...
inline bool SiStripFedCabling::ConnsRange::empty() const { return (range_.begin() == range_.end()); }

inline uint32_t SiStripFedCabling::ConnsRange::size() const { return std::distance(range_.begin(), range_.end()); }

inline SiStripFedCabling::ConnsPair SiStripFedCabling::ConnsRange::connsPair() const {
  return (
      (range_.begin() == vector_.end() && range_.end() == vector_.end())
          ? ConnsPair(sistrip::invalid32_, sistrip::invalid32_)
          : ConnsPair(std::distance(vector_.begin(), range_.begin()), std::distance(vector_.begin(), range_.end())));
}
