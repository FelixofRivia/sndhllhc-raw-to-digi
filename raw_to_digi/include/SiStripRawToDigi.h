    ...
    for (const auto fed_id : fed_ids) {
        ...
        for (uint8_t i_ch{0}; i_ch < FEDCH_PER_FED; ++i_ch) {
            ...
            while (offset != end) {
                ...
                if (!((module_strip_id % 128) <= 5 || (module_strip_id % 128) >= 123) && !(module_strip_id<10 || module_strip_id > 758 )) digis.emplace_back(SiStripDigi(module_strip_id, getADC_W<num_words>(data, offset, bits_shift), fed_key, 0, detector_info));
